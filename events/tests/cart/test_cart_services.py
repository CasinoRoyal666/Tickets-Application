...
class TestCartService:
    ...
    @pytest.mark.parametrize("get_event_id, quantity, expected_error", [
        (lambda e: e.id, 100, "Not enough tickets. Available: 50"),
        (lambda e: 999, 1, "Event does not exist"),
    ])
    def test_validate_cart_item_invalid(self, sample_events, get_event_id, quantity, expected_error):
        event = sample_events[0]
        event_id = get_event_id(event)
        is_valid, error = CartService.validate_cart_item(event_id, quantity)
