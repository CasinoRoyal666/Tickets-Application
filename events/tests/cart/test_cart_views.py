...
class TestCartViewSet:
    ...
    def test_add_item_to_cart(self, api_client, sample_events):
        ...
        assert response.data['total_price'] == Decimal('300.00')  # 100 * 3
    
    def test_add_item_invalid_event_id(self, api_client):
        url = reverse('cart-list')
        data = {
            'event_id': 999,
            'quantity': 1
        ...
        assert 'Event does not exist' in str(response.data)
    
    def test_add_item_invalid_quantity(self, api_client, sample_events):
        url = reverse('cart-list')
        data = {
            'event_id': sample_events[0].id,
            'quantity': 0
    ...
    def test_add_item_not_enough_tickets(self, api_client, sample_events):
        url = reverse('cart-list')
        data = {
            'event_id': sample_events[0].id,
            'quantity': 100 
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert 'Not enough tickets' in str(response.data)
    ...
    def test_get_cart_with_items(self, cart_with_items, sample_events):
        ...
        assert response.data['total_price'] == Decimal('350.00')  # 2*100 + 1*150
